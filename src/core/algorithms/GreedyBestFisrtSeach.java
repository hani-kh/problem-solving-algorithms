...
public class GreedyBestFisrtSeach extends SearchAlgorithm {
    ...
    // PriorityQueue is the data structure for implementing All BestFirstSearch Algo. Containers.
        ...
        while (!(frontier.isEmpty())) {
            ...
            for (IAction action : p.getActions()) {// for each action defined in problem we need to apply this action to the current state
                ...
                for (State s : nextStates) {// for each state in the result of applying action to the current state.
                    ...
                    // compute the estimated cost f(n) = h(n).
                    double totalEstimatedCost = p.getHeuristicFunction().getH(s);
                    // create a new node and fill information.
                    Node child = new Node(s, currentNode, action, pathCost, totalEstimatedCost);
                    frontier.add(child);// add the new node to frontier.
                    //add node and it's parent to search tree.
                    // this for printing the search tree diagram.
