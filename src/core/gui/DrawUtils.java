...
public class DrawUtils {
    ...
    public static boolean isOnEdge(MouseEvent e, GUIEdge edge) {
    ...
    public void drawWeight(GUIEdge edge) {
    ...
    public void drawPath(List<GUINode> path) {
        List<GUIEdge> edges = new ArrayList<>();
        for(int i = 0; i < path.size()-1; i++) {
            edges.add(new GUIEdge(path.get(i), path.get(i+1)));
        }
    ...
    public void drawPath(GUIEdge edge) {
        g.setColor(parseColor("#00BCD4"));
        drawBoldEdge(edge);
    }

    public void drawHoveredEdge(GUIEdge edge) {
        g.setColor(parseColor("#E1BEE7"));
        drawBoldEdge(edge);
    }

    private void drawBoldEdge(GUIEdge edge){
        Point from = edge.getNodeOne().getCoord();
        Point to = edge.getNodeTwo().getCoord();
    ...
    public void drawEdge(GUIEdge edge) {
        g.setColor(parseColor("#555555"));
        drawBaseEdge(edge);
        drawWeight(edge);
    }

    private void drawBaseEdge(GUIEdge edge){
        Point from = edge.getNodeOne().getCoord();
        Point to = edge.getNodeTwo().getCoord();
    ...
    public void drawHalo(GUINode node){
    ...
    public void drawSourceNode(GUINode node){
        g.setColor(parseColor("#00BCD4"));
        g.fillOval(node.getX() - radius, node.getY() - radius, 2 * radius, 2 * radius);
        ...
        radius+=5;
        g.setColor(parseColor("#00BCD4"));
        drawCentreText(String.valueOf(node.getId()) +"/"+String.valueOf(node.getHeuristicValue()) , node.getX(), node.getY());
    }

    public void drawDestinationNode(GUINode node){
    ...
    public void drawNode(GUINode node){
